...
type Request = { id: string; source: string; sourceCardId?: string; target: string; cardId: string; tool: string; text?: string; key?: string; submit: boolean; expiresAt: number };
export function McpApprovals() {
  ...
  const decide = async (approve: boolean) => {
    ...
    try {
      const result = await invoke<{ error?: string }>("mcp_decide", { id, approve });
  ...
  return <dialog ref={dialog} className="cq-urgent-dialog cq-mcp-approval" role="alertdialog" aria-labelledby="cq-mcp-approval-title"
    ...
    {active && <>
      ...
      <p>{t(active.tool === "start_card" ? "mcp.startHint" : "mcp.approvalHint")}</p>
      <dl>
