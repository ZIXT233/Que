...
import { parseWorkspaceEnvironment } from "./WorkspaceEnvironmentEditor";
export function WorkspaceRcEditor({ machineKey, value, environment, onChange }: { machineKey: string; value: string; environment: string; onChange: (value: string) => void }) {
  const { t } = useI18n();
  ...
  async function verify() {
    ...
    try {
      const sessionEnv = parseWorkspaceEnvironment(environment);
      if (!sessionEnv) { setResult(t("workspace.envInvalid")); return; }
      const response = await fetch("/api/card-queue", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "workspace_rc_verify", machineKey, terminalRc: value, sessionEnv }) });
      const data = await response.json();
      setResult(response.ok ? data.output : data.error || data.code || t("workspace.rcFailed"));
    } catch (error) { setResult(String(error)); }
    finally { setBusy(false); }
  }
  return <details className="workspace-advanced-setting">
    <summary>{t("workspace.rcTitle")}</summary>
