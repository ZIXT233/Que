...
export function CardQueueShell() {
  ...
  const renderCardContent = (visibleCard: QueueCard, isFront = true, layout?: DetachedCardLayoutControls) => {
    ...
    <article aria-hidden={!isFront} inert={!isFront} className="cq-large-card cq-continuous-card" data-transfer-id={visibleCard.id} data-transfer-zone="deck" data-card-id={visibleCard.id} data-phase={visibleCard.phase} data-working-view={visibleCard.phase === "working"} data-urgent-call={hasUrgentCall(visibleCard)}>
              ...
              </ScoreFormulaPopover>}</div><div className="cq-card-title-row"><div className="cq-title-primary">{workspace?.kind === "ssh" ? <ScoreChipTooltip text={<div className="cq-environment-tooltip"><span><WorkspaceMachineIcon name="remote" size={14} />{hostLabel}{isTmux && <span className="cq-tmux-badge">TMUX</span>}</span><small>{remoteAddress}</small></div>}><span className="cq-title-environment cq-title-host" aria-label={hostWithHarness}><WorkspaceMachineIcon name="remote" size={15} />{isTmux && <span className="cq-tmux-badge">TMUX</span>}<b title={hostWithHarness}>{hostWithHarness}</b></span></ScoreChipTooltip> : <span className="cq-title-environment cq-title-host" aria-label={hostWithHarness}><WorkspaceMachineIcon name="local" size={15} /><b title={hostWithHarness}>{hostWithHarness}</b></span>}<ScoreChipTooltip text={<div className="cq-environment-tooltip"><span><WorkspaceMachineIcon name="folder" size={14} />{workspaceLabel}</span><small>{workspace?.cwd || visibleCard.cwd}</small></div>}><span className="cq-title-environment cq-title-workspace" aria-label={workspaceLabel}><WorkspaceMachineIcon name="folder" size={15} /><b>{workspaceLabel}</b></span></ScoreChipTooltip></div><div className="cq-title-controls"><div className="cq-title-harness" /><div className="cq-title-branches" /><div className="cq-title-actions"><div className="cq-title-tools">{layout ? <SideTerminalButton pressed={layout.terminalOpen} label={t("queue.sideTerminal")} onClick={layout.toggleTerminal} /> : sideButton}</div>{(placementButton || visibleCard.harness) && <div className="cq-card-more">
                <button type="button" className="cq-tools-trigger" aria-label={t("queue.more")} aria-haspopup="true"><span aria-hidden="true">···</span><span>{t("queue.more")}</span></button>
