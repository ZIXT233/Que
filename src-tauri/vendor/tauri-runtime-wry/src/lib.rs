...
pub struct Context<T: UserEvent> {
  ...
  #[cfg(windows)]
  main_thread: main_thread_context::Handle<DispatcherMainThreadContext<T>>,
  #[cfg(not(windows))]
...
impl<T: UserEvent> Context<T> {
  #[cfg(windows)]
  fn main_thread(&self) -> Rc<DispatcherMainThreadContext<T>> {
    self.main_thread
      .get()
      .expect("runtime context requires its live owner thread")
  }

  #[cfg(not(windows))]
  fn main_thread(&self) -> &DispatcherMainThreadContext<T> {
    &self.main_thread
  }
    ...
    F: FnOnce(Option<&DispatcherMainThreadContext<T>>) -> R,
  {
    #[cfg(windows)]
    {
      f(self.main_thread.get().as_deref())
    }
    #[cfg(not(windows))]
    {
...
pub struct Wry<T: UserEvent> {
  context: Context<T>,
  #[cfg(windows)]
  // Drops on the event-loop thread before event_loop. Worker handles never own
  // this payload and cannot postpone its destruction to a worker thread.
  _main_thread_owner: main_thread_context::Owner<DispatcherMainThreadContext<T>>,
...
impl<T: UserEvent> RuntimeHandle<T> for WryHandle<T> {
  ...
  fn request_exit(&self, code: i32) -> Result<()> {
    #[cfg(windows)]
    if !self.context.main_thread.is_alive() {
      return Err(Error::FailedToSendMessage);
    }
  ...
  fn monitor_from_point(&self, x: f64, y: f64) -> Option<Monitor> {
    (|| -> Result<_> {
      event_loop_window_getter!(self, |sender| EventLoopWindowTargetMessage::MonitorFromPoint(sender, x, y))
    })().ok().flatten()
  }
...
impl<T: UserEvent> Wry<T> {
  ...
  fn init(event_loop: EventLoop<Message<T>>) -> Result<Self> {
    ...
    let main_thread = DispatcherMainThreadContext {
    ...
    #[cfg(windows)]
    let main_thread_owner = main_thread_context::Owner::new(main_thread);
    let context = Context {
      window_id_map,
      main_thread_id,
      proxy: event_loop.create_proxy(),
      #[cfg(windows)]
      main_thread: main_thread_owner.handle(),
      #[cfg(not(windows))]
      main_thread,
    ...
    Ok(Self {
      context,
      #[cfg(windows)]
      _main_thread_owner: main_thread_owner,
      event_loop,
    })
  }
}
  ...
  match message {
    ...
    Message::EventLoopWindowTarget(message) => match message {
      EventLoopWindowTargetMessage::PrimaryMonitor(sender) => {
        let _ = sender.send(event_loop.primary_monitor().map(|m| MonitorHandleWrapper(m).into()));
      }
      EventLoopWindowTargetMessage::MonitorFromPoint(sender, x, y) => {
        let _ = sender.send(event_loop.monitor_from_point(x, y).map(|m| MonitorHandleWrapper(m).into()));
      }
      EventLoopWindowTargetMessage::AvailableMonitors(sender) => {
        let _ = sender.send(event_loop.available_monitors().map(|m| MonitorHandleWrapper(m).into()).collect());
      }
...
"#,
  ...
  let main_thread = context.main_thread();
  let mut web_context = main_thread
    .web_context
    .lock()
    ...
    webview_builder = webview_builder.with_new_window_req_handler(move |url, features| {
      ...
      match response {
        ...
        tauri_runtime::webview::NewWindowResponse::Create { window_id } => {
          #[cfg(windows)]
          let Some(main_thread) = context.main_thread.get() else {
            return wry::NewWindowResponse::Deny;
          };
          #[cfg(not(windows))]
          let main_thread = context.main_thread();
          let windows = &main_thread.windows.0;
          let webview = windows
            .borrow()
            .get(&window_id)
            .unwrap()
